        ...
        btnSave.setOnClickListener {
            userRepository.getUser(editTxtUsername.text.toString(), onSuccess = { user ->
                if (editTxtUsername.text.toString().isEmpty() ||
                    editTxtCurrentPassword.text.toString().isEmpty() ||
                ...
                else if (user.username != StoredDataHelper.get(this))
                    Toast.makeText(this, "That username is taken. Try another.", Toast.LENGTH_SHORT)
                        .show()
                else {
                    uploadEditedProfile(imgName, imgUri)
                    val storedUsername = StoredDataHelper.get(this)
                    userRepository.getUser(storedUsername, onSuccess = { user ->
                        Log.d("Hashed password", user.password)
                        Log.d("Plain password", editTxtCurrentPassword.text.toString())
                        if (!BCrypt.checkpw(
